...
export default function SongCard(props: {
  ...
  return (
    <div className="glass group flex min-w-0 gap-3 overflow-hidden rounded-2xl p-3 shadow-glow transition hover:bg-white/10 sm:gap-4 sm:p-4">
      <div className="flex w-8 shrink-0 flex-col items-center justify-center sm:w-14">
        <div className="font-display text-base tabular-nums sm:text-xl">{song.currentRank ?? '—'}</div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5 sm:mt-3 sm:gap-2">
          <button
            className={clsx('btn h-9 px-3 text-sm sm:h-10 sm:px-4', isVoted ? 'btn-voted' : 'btn-primary', popped && 'animate-pop')}
            onClick={handleVote}
            disabled={busy || isVoted}
          >
            {busy ? 'Voting…' : isVoted ? 'Voted' : 'Vote'}
            <span className="rounded-lg bg-black/25 px-1.5 py-0.5 text-xs tabular-nums">{song.votes}</span>
          </button>

          {song.youtubeUrl ? (
            <button
              type="button"
              className="btn btn-ghost h-9 px-2.5 text-xs sm:h-10 sm:px-3"
              onClick={() => onOpenMedia('youtube', song.youtubeUrl || '', `${song.title} - ${song.artist}`)}
              aria-label="YouTube"
              title="YouTube"
            >
              <span className="hidden sm:inline">YouTube</span>
              <svg className="sm:hidden" width="17" height="17" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          >
            ...
            <svg className="sm:hidden" width="17" height="17" viewBox="0 0 24 24" fill="none" aria-hidden="true">
